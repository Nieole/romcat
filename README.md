...
**<!-- 数:票数 -->149/152 张票落地**（全仓 `.scratch/*/issues/` 里一份文件一张票数出来的）。没落地的分两类：`ready-for-human` 的那几张要人上真机——一张要在 Windows 真机上核对路径形式与文件元数据，一张要在真库上跑一趟把数字补进事实台账（Windows 真机中文输入法验证 2026-09-13 已过，界面层留在 egui）；其余是后几条队列里还没开工的：界面观感照设计稿落地、一个判断一处实现、六步都在界面上答完、开一份库不留哑口、机器替人核前提。

**<!-- 数:测试条数 -->2,615 条测试、<!-- 数:测试目标数 -->86 个测试目标**（口径就是门禁 `test` 那一条加 `-- --list`；它到底跑什么，`cargo xtask gate --list` 当场印），**<!-- 数:ADR份数 -->24 份架构决策记录**。
