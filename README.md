...
**87/90 张票落地**（六条队列合计）。余下三张全是 `ready-for-human`：两张 Windows 真机验证（需要人在 Windows 上跑），一张要在真库上跑一趟把数字补进事实台账。**1,815 条测试**（`cargo test --workspace --all-features -- --list` 数出来的，70 个测试目标；**这个数会随票涨，以门禁自己的输出为准**，别拿这里当基线——这一次量在 `main` `a142612` 上，第四轮九张票全部合并之后）。
