...
//! 地方，漏跑了没有人拦，本地与 CI 也无从对齐（挂单 `Q187`）。现在门禁的每一条只在下面 [`steps`]
...
/// 挂单 `Q189`）——那时门禁里只有这一条从来没写过标准，`clippy` 那条明写「零警告」，它没有。
...
/// 交付出去的那份配置还编得过。**编整个工作区的那几条里，唯一不带 `--all-features` 的一条。**
///
