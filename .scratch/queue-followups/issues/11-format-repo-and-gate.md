...
### 追加：`Q189`（票 `parking-3/02` 落地）

本票收尾时第 5 条审查意见让 `cargo doc` 头一次真跑了一趟，如实标了带保留：
**68 条既有 rustdoc 告警 → `Q189`**。那一条由票 `parking-3/02` 收掉，**settled**。

| 挂单 | 它说的 | 裁决 |
|---|---|---|
| `Q189` | `cargo doc` 那一条从来没绿过——68 条 rustdoc 告警全是既有的（`romcat-core` 独占 56 条），退出码是 0，「跑绿」只在字面上成立；四条门禁里只有它从来没写过标准 | **settled。** 清到 **0** 并敲成硬红：`xtask/src/gate.rs` 的 `DOC_ENV` 换成 `&[("RUSTDOCFLAGS", "-D warnings")]`，命令补上 `--all-features`。从此断一条文档链接门禁当场红，**这件事有测试钉着**（`xtask/tests/gate.rs` 的 `文档里断一条链接就红改成不带链接的写法就绿`，与 `Q187` 那条红绿测试同一个写法：真起子进程、两面都验）。清的那 67 条分四类——多余的显式链接目标 7、解析不了的 intra-doc 链接 4、「既是函数又是模块」的歧义 3、公开文档链到私有条目 53；最后一类一律改成不带链接的代码体，**没有**把任何私有条目改成公开（代价记在 `Q207`）。 |

**顺带更正本票记录的两处数：**

- **不是 68 条，是 67 条**（`romcat-core` 58 / `romcat-gui` 8 / `romcat-cli` 1），
  也不是「`romcat-core` 独占 56」。本票当年那个 68 是当时的实测，中间十几张票各自
  添过又清过，数一直在动。原句不改——那是当时的实测，改掉就没有对照了。（`Q206`）
- **门禁现在是五条不是四条。** `doc` 补上 `--all-features` 之后，四条里就没有一条再
  编译 `demo` 关掉的那份交付配置了（票 `parking-3/01` 收尾审查报的第 1 条，
  它把告示留在了 `DOC_ENV` 的注释上）。票 `parking-3/02` 另加一条
  `cargo check --workspace`（默认特性）把那一格接住，取舍与代价记在 `Q204`。
