...
## 三修订：NKit 那一处是**逻辑**偏移；CHD 的负载这一层还够不到（票 09）

本 ADR 上面写「判据：光盘逻辑偏移 `0x200` 处 4 字节 == `"NKIT"`」，**逻辑**两个字在票 07
落地时被读成了文件偏移。差别在压缩镜像上是致命的：

- `.nkit.gcz`、WBFS、CSO 里那一处**不在文件的 `0x200` 上**，要先按各自的壳子打开
  才读得到。按文件偏移验，这几种一律验不出来，而验不出来又被记成了「没验过」——
  结论侥幸不错，但那是运气，不是判据。
- 票 09 把 NKit 挪进 `identify::disc`：先认壳子、再取逻辑光盘的那一段、最后读 `0x200`。

**够不到那一处的仍然是「没验过」，不是「不是」。** 真机上 WBFS 只交得出 256 字节的
Wii 光盘头、WIA/RVZ 只交得出 `0x80` 字节，两种都够不到 `0x200`——它们的命中一律降一档。

### CHD 的负载这一层不碰

`chdman info` 那两个 SHA-1 对不上 Redump（上面那段修订已经说过），而**盘内的
`SYSTEM.CNF` 要先解 CHD 的 hunk map 再解一个 hunk 才读得到**。map 是 Huffman 编码的，
hunk 的编解码器是 `cdlz` / `cdzl` / `cdfl`（LZMA / Deflate / FLAC）——那是一整套解码器，
不是「读几百字节」。票 09 因此只读 CHD 的头（版本、逻辑流长度），如实报「这一层还不做」。
代价是真机上 PS1 的 126 份 `.chd` 拿不到序列号，记在挂账里。

## 四修订：序列号这种弹药**按平台严重不均**（票 09 实测）

「压缩镜像反而比归档更好识别」这句话只说对了一半——**读得出标识**与**撞得上**是两件事。
真机把 395 份 DAT 重新解析一遍，索引出 95,022 条序列号，分布是：

| 有弹药 | 没弹药 |
|---|---|
| PSV 35,242、NDS 15,232、PSP 8,627、3DS 7,288、GBA 6,234、PS1 3,156 | **PS2 178、NGC 58、WII 27、WIIU 1、PS3 0** |

原因是**Redump 的官方 DAT 一条序列号都不写**（实测逐字确认：`<game>` 上只有 `name` 与
`id`，`<rom>` 上只有四套哈希）。而 PS2 / NGC / WII 的主力弹药正是 Redump。于是这三个
平台的镜像**读得出 Game ID 与 `SLPS-25604`，却无处可撞**。

两条推论：

- **报告必须把「序列号弹药」单列一列**，与票 07 那条「DAT 条目」同理：没有弹药的平台
  命中率低，与识别准不准无关。
- 序列号写在 `<rom serial>` 属性与 `<game_id>` 子元素上，**不在 `<game serial>` 上**。
  票 06 的解析器只读后者，于是 Vita 与 PSP 两个平台一条都读不到。这一条是「一个字段
  读漏了，整个平台的弹药归零」的教科书例子。
