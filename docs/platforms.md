...
| GBA | ✅ | game code `AGB-XXXX`；汉化补丁通常不改 |
...
| 平台 | 已有调研 | 备注 |
|---|---|---|
| FC | ✅ | iNES / NES 2.0 头；GoodNES 有 646 条 `[T+Chi]` |
...
**`pc` 本期不做。** 它是容量最大的一块（数见 `library-facts.md`「被推翻的判断」），但不在原议定的 27 平台里，用户也不要求导出。明确排除，不是遗漏。

**`switch` 纳入。** 它比 PS3 晚一代，原清单没有（容量与扩展名构成见 `library-facts.md`「被推翻的判断」）。调研见 `research/switch-identification.md`。**票 27 已落地**（免密钥路线：容器层的明文文件名表，`identify::switch`）——认出多少见 `library-facts.md`「Switch 免密钥识别」。

**`N-Gage` 维持跳过**，归入「其他掌机」兜底桶。

**卡带世代的开工顺序按成型后的变体数定，不按文件数。** 任务重不重取决于要识别多少个变体，而不是要遍历多少个文件——这两个数在本库里差得极远（数见 `library-facts.md`）：PSV 的文件多却远不到那么多游戏；FC 的文件绝大多数是透明容器，一个归档可能是一个 ROM 也可能是一整个合集。变体数由票 05 产出，05 → 07 → 10 的依赖顺序保证排到票 10 时它已经存在。

库体检的文件数只能当粗略上界（几个卡带平台的文件数记在 `library-facts.md`「规模」）。

> **变体数出来了**（2026-09-01，票 05），按平台的全表见 `library-facts.md`「按平台的变体数」。
>
> **PSV 的排位整个变了**：按文件数它是全库第一，按变体数它和 GBA 一个量级。

此前 research 给的 P0/P1/P2 排序基于资源站的全站分布，**不适用于这个库**——最刺眼的例子是街机：全站条目上万，这个库里却只是零头（两边的数见 `scale-reference.md` 与 `library-facts.md`「被推翻的判断」）。

**PSV 的分量被严重低估了。** 它是全库文件数最多的平台（数见 `library-facts.md`「被推翻的判断」），是票 05 成型规则最吃重的地方，而非原先以为的「一个成本 8.5 人日的边缘平台」。
