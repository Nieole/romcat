...
// ——— 子库 ———
//
// 子库屏那几张拍的是**整个窗口**（`App`：顶栏连子库屏）。屏上画着的每一样都得是定值（票
...
/// **超限的一台**那一对：卡片顶边贴着可视区顶部（不滚），删减表整张在视口里。
///
/// 早先是把卡片那一列滚到底再拍：滚动位置夹在最大那一格上，图顶上「选择集 · 1 条规则」那一行只露出下半截，
/// 字被切掉一半、看着像画坏了（协调人对稿打回，票 `gui-looks-like-the-design/20` 第二段）。另一种停法——
/// 「容量」标题贴着顶——走不到：那要比最大那一格还往下滚。卡片顶边整个露出来时删减表照样看得全。
///
