...
fn 拍浏览(名字: &str, 主题: Theme, 态: 浏览态) {
    ...
    // [`带标签的行正题露得出字`] 查的是**表格那几行**被截成什么样：筛空那一态一行都不剩，
    // 卡片那一态屏上根本没有表格——卡面画的是标题、一行「年份 · 变体 · 大小」与置信度那一句，
    // 「未关联作品」那枚标签只有表格画（`table.rs` 的 `two_lines`）。这两态跳过。
    //
    // **卡片墙该不该另有一个「未关联作品」的记号，是稿上没画、拿主意的人还没裁的一件**：
    // 眼下它靠置信度那一句（「没有候选」）说这件事。这里只是不拿表格的尺子去量卡片。
