...
fn 拍合并向导(名字: &str, 主题: Theme, 第几步: usize) {
    ...
    // **步骤条贯通左右、三格等宽**（拿主意的人 2026-09-21 定，**与设计稿不同**）。
    //
    // 这里断的是**三段之间的几何**，不写死弹层摆在哪儿：头两问是「圆点 + 名字」靠左摆，
    // 两问名字的左缘之差就是一格的宽；末一问靠右摆，于是
    // 「头一问那一格的左缘 + 三格」就该正好落在末一问名字的右缘上。这一条同时钉住两件事
    // ——几格等宽，而且末一问贴着它那一格的右缘（也就是整条贯到头）。
    //
    // **「那一格的右缘就是这一层的右内缘」另有一条钉着**
    // （`tests/dialog.rs::走到第几问那一排贯通左右而且几格等宽`）：那一条量得到
    // `Shown::rect`，绝对位置在那儿断。这里不重复写死坐标——弹层摆在哪儿不是这一票的事。
    {
        ...
        let 圆点那一截 = romcat_gui::tokens::Tokens::builtin().layout.page_dot + look::step(1);
        let 末格右缘 = 每一问[0].left() - 圆点那一截 + 3.0 * 格宽;
        assert!(
            (每一问[2].right() - 末格右缘).abs() < 1.0,
            "步骤条没贯通左右：末一问的右缘在 {}，按三格等宽该在 {末格右缘}（一格 {格宽} 宽）",
            每一问[2].right(),
        );
    }
