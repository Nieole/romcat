...
//!
//! 这几条验的是那条路本身，而不是它跑完之后的六屏。从前它整条摊在 `main.rs` 里，
...
fn 给一份现成的库就走完启动进主窗口() {
    ...
    // 主窗口真的画出来了：左栏上六屏的名字都在。
    let ctx = headless::context();
    let out = headless::frame(&ctx, headless::input(), |ui| program.ui(ui));
    let 屏上 = shared::画出来的字(&out);
    for 屏 in ["待确认", "库", "浏览", "子库", "任务", "设置"] {
...
fn 开过一份库之后再启动直接进主窗口不经过开场() {
    ...
    // **工作目录是从那条路径反推出来的**：状态栏右边写着它在哪儿——反推错了这一句就指到别处去了
    // （验收第 2 条）。
    //
    // 认的是**状态栏那一处**：从前认的是待确认屏屏头上「沉淀库 …」那一句（沉淀库跟着工作目录走，
    // `工作目录/verdict/…`），票 `gui-looks-like-the-design/18` 照稿把那一句挪进了裁决记录抽屉的页脚，
    // 屏头上不再有它。状态栏那一处**把家目录缩成 `~/…`**（`App::shorten_home`），所以两种写法都认：
    // 临时目录落在家目录底下（本机的 `TMPDIR` 正是）时画的是缩过的那一种。
    let 工作目录写法 = {
        let 缩过的 = std::env::var_os("HOME").and_then(|home| {
            工作目录
                .path()
                .strip_prefix(&home)
                .ok()
                .map(|剩下的| std::path::Path::new("~").join(剩下的))
        });
        缩过的.map_or_else(
            || romcat_core::path::display(工作目录.path()),
            |短| romcat_core::path::display(&短),
        )
    };
    assert!(
        屏上.contains(&工作目录写法),
        "反推出来的不是那份库住的工作目录（该写着 {工作目录写法}）：\n{屏上}",
    );
...
///
/// 「报得出进度、按得下停止」只有在扫描还**在跑**的时候才验得到，而这几条是从界面进去
/// 的——按一下要跑四帧（找位置、按下、松开、再画一帧），实测每帧几十毫秒。四百个文件
/// 那块盘 121 毫秒就扫完了，正好落在那四帧里，于是屏上只剩一条历史；后来加到三千个。
///
/// 进主窗口那头几帧要头一回排版、光栅化一批新字形，三千个文件又赶在「停止」按下去之前收了场：
/// 主窗口长出左栏与屏头（票 `gui-looks-like-the-design/32`）之后那一趟 185 毫秒扫完，那张票加到
/// 一万个；任务屏照稿重排（票 `gui-looks-like-the-design/25`，挂单 `Q833`）之后头两帧实测 289 与
/// 231 毫秒，那张票加到三万个。两样合进来取大的那个，三万个，扫描拉到两秒上下。**这仍是一场
/// 与挂钟的赛跑**，真扫描没有等信号的口子（挂单 `Q864`）。**只往临时目录里写**（ADR-0004）。
...
fn 走完向导之后扫描已经排在任务台上报得出进度也按得下停下() {
    ...
    // 收场之后历史里留一行——**那一趟真的排上去过**，不是屏上画了一行好看的。屏头上那句
    // 「N 条历史」照稿不要了（票 `gui-looks-like-the-design/25`），改看历史表不是空的。
    assert!(
        !屏上.contains("还没有已完成的任务。"),
        "台上没留下这一趟：\n{屏上}"
    );
