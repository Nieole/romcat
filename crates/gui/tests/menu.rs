...
/// **做不到的那两项一项都不摆**（`browse::menu` 那一节，挂单 `Q1140`）。
///
/// 摆一项按下去没有下一步的，等于屏上写着一件做不到的事。
///
/// **欠的是谁，这半年变过一次**：写这一条时两层弹层都没做，理由是「弹层还没做出来」；
/// 票 `gui-looks-like-the-design/13` 把「加入合集」那层做出来之后，**那句理由不再成立**
/// ——今天不摆的理由是没人往 `Open::lines()` 那份清单里添这一项，而添不添是 `Q1140`
/// 上挂着、拿主意的人还没裁的事。「加入子库…」照旧等票 `23`。
/// 两项哪天摆上去了，这一条该翻成正面断言（`Q1140` 的「建议留」那一段写着）。
///
/// **为什么要两帧相减，不照整屏断言**：`右键` 交回的是**整屏**的字，而「加入合集…」
/// 这一句在表格上方那一条的按钮上也写着（票 13 加的）。照整屏断言的话，屏上别处多出
/// 同名的一句就把这一条判红——它测的就不再是菜单了。这一条 2026-09-22 正是这么红的。
/// 拿「菜单摊着」与「按 Esc 收掉之后」两帧相减，**差出来的才是菜单自己那几项**；
/// 选中哪一行这类右键顺带改掉的状态，两帧里是同一份，减掉了。
#[test]
fn 做不到的那两项一项都不摆() {
    let ctx = headless::context();
    let mut app = 界面();
    ...
    // 浮层收掉也要几帧才摆稳，同 `右键` 里那一段。
    let mut 收掉 = String::new();
    for _ in 0..4 {
        收掉 = 跑(&ctx, &mut app, Vec::new());
    }
    let 数几处 = |屏上: &str, 那一句: &str| 屏上.matches(那一句).count();
    // **先拿一项确实在菜单上的来验这把尺**：相减减不出东西的话（Esc 没收掉、或者
    // 两帧取的不是同一份状态），底下那两条会因为**永远减出 0** 而永远绿。
    assert_eq!(
    ...
    for 不该有的 in ["加入合集…", "加入子库…"] {
        let 菜单摆了几处 = 数几处(&摊着, 不该有的) - 数几处(&收掉, 不该有的);
        assert_eq!(
            菜单摆了几处, 0,
            "菜单上不该有「{不该有的}」——它按下去没有下一步（挂单 `Q1140`）：\n{摊着}"
        );
    }
}
