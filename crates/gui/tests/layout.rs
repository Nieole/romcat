...
fn 拖到极限时不塌陷也不把正中那块挤没() {
    ...
    // 头一个是**左栏那块还在**的证据。原先取的是搜索框底下那句排序说明，可那一句
    // 2026-09-21 起**只在搜索框里有字时才画**（挂单 `Q1099`），而这条测试没搜索——
    // 拿它当证据的话，左栏真塌了也验不出来。
    //
    // 换成左栏抬头上那颗「清除」：**它画在滚动区外头**，拖到最窄也还在。
    // （「条件组」不行——拖到最窄时那一栏的内容变高，它被滚动区剔到视口外，实测拿不到。）
    for 抬头 in ["清除", "变体", "元数据"] {
        assert!(
            屏上.contains(抬头),
            "往里拖到底之后「{抬头}」那一块没了：\n{屏上}"
...
#[test]
fn 导航在左栏_三组六个入口照稿排好_顶栏上的换一份库不在了() {
    // 规格「测试决定」：导航在左栏且入口齐。**设置自票 `gui-looks-like-the-design/31` 起在了**，
    // 摆在「后台」那一组、紧跟任务（照稿）；新手引导主程序眼下仍没有，那一颗照旧不摆。
...
fn 点左栏入口就换到那一屏_屏头写着那一屏_右侧是它原来在顶栏上的那一段() {
    ...
    for (view, 入口, 右侧那一段) in [
        ...
        (View::Library, "库", Some(romcat_gui::roots::ADD_ROOT)),
        (View::Queue, "待确认", Some("重新列队列")),
        // **浏览屏的屏头右侧照稿是空的**（拿主意的人 2026-09-22 对着稿裁，挂单 `Q1102`）：
        // 稿上 `#s-browse .scrhead` 里只有标题与副标题，那几颗批量按钮从来就在表格上方
        // 那一条的右端（`.tbar .acts`）。票 09 把它们挪进过屏头（`Q876`／`Q877`），这一票挪回去了。
        (View::Browse, "浏览", None),
        (View::Sublibraries, "子库", Some("重新列一遍")),
        // 任务屏照稿重排之后（票 `gui-looks-like-the-design/25`）右侧只放「清空历史」，顶栏那句摘要照稿不要了。
        (View::Tasks, "任务", Some("清空历史")),
    ] {
        ...
        if let Some(右侧那一段) = 右侧那一段 {
            ...
            // **照稿是空的那几屏，断的是「真没有」**：只跳过上面那条断言的话，
            // 一份把按钮又搬回屏头的实现照样全绿。
            //
            // ⚠️ **比的是「整段落在屏头下沿之下」，不是「不在两行屏头那一片里」**：
            // 屏头右侧一空它就只有一行高，而表格上方那一条紧跟在它底下——拿
            // [`折成两行的屏头底`] 当界线的话，那一条整个落在界线以内，
            // 于是**摆对了位置的按钮反而被判成「还在屏头里」**（第一版就是这么红的）。
            for 不该在屏头里 in ["刮削…", "★ 收藏", "合并作品…"] {
                let 画在 = 画在哪几处(&out, &|画的| 画的.contains(不该在屏头里));
                assert!(
                    !画在.is_empty(),
                    "{view:?} 上压根没有「{不该在屏头里}」——它该在表格上方那一条的右端：\n{}",
                    画出来的字(&out),
                );
                assert!(
                    画在.iter().all(|rect| rect.top() > 屏头底()),
                    "{view:?} 的屏头里还摆着「{不该在屏头里}」（画在 {画在:?}，屏头下沿 {}）——\
                     照稿它在表格上方那一条的右端：\n{}",
                    屏头底(),
                    画出来的字(&out),
                );
