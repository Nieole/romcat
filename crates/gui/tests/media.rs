...
/// 合成数据的规模。**形状照真库，个头不照**。
///
/// 形状那一半是白拿的：`demo::browse` 现在按真库的比例折作品数（`demo::works_for`），
/// 两千个变体收出三百多个作品，一个作品名下挂着六七个变体——与真库那 46,428 → 7,407
/// 是同一副样子，而媒体本来就是**挂在作品那一层**的。
///
/// 个头没跟上去是**量过之后的取舍**：推到 `demo::BROWSE_VARIANTS` 这九条一条不改照样
/// 全绿（试过），可这份测试文件从 **2.5 秒变成 45.9 秒**——它钉的是解没解码、画没画出来、
/// 池子在不在位，没有一条跟着库有多大变。挂单 `Q356` 记着这一条。
...
fn 切换选中那一帧不解码() {
    ...
    //
    // ⭐ **判据是「解没解码」，不是「花了多久」。** 这里原先还有第三条断言：头一帧的
    // **挂钟**耗时得小于一秒。它在门禁上是一张彩票——实测同一棵树、同一份代码，
    // `--test-threads=6` 且机器上另有三个工作区在冷编译时头一帧 1.82s、红；
    // `--test-threads=2` 那一趟、以及安静下来单跑的三趟，都是绿
    //（票 `parking-3/01` 落门禁那天量的，挂单 `Q196`）。它量的是这条测试
    // **抢不抢得到 CPU**，不是这段代码有没有在画帧线程上解码——而 CI 的 runner
    // 比这台又慢又忙。
    //
    // 下面那两条断言是**结构性**的，本来就把上面那句话说全了：头一帧一张图都没解出来
    // （`ready() == 0`），而活确实排出去了（`busy() > 0`）。**要量时间这个仓库有专门的
    // 地方**：`--bench` / `--bench-browse` 那几条（`demo` feature 后面）。
    let ctx = headless::context();
