...
//!
//! - **加入合集…**——那层弹层是票 `gui-looks-like-the-design/13`（收藏与合集）的。今天
//!   合集只在筛选栏底下那个格子里加得了，菜单里点一下没有下一步。
...
/// `window_stroke`（挂单 `Q1141`）。理由是稿上那一层多半落在页面底色上，而这一层
/// **落在哪儿由右键按在哪儿定**——落到右边那块详情栏上时，它与身底下那一块是**同一个
/// 颜色**（两处都是令牌 `panel`），暗色里除了一点阴影什么边都看不出。一层看不出边界的
/// 浮层，人分不清哪几行属于菜单。仓里每一层浮起来的东西都带这一圈（`dialog.rs` 走的
/// 也是 `window_stroke`，egui 自己的 `Frame::popup` 同样），所以这不是新发明一种画法。
