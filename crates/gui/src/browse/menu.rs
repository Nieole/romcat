...
//! 按不动的**不摆**。
//!
//! ⚠️ **稿上那十项这会儿一项不缺**（票 `gui-looks-like-the-design/23`，挂单 `Q1140` 收口）。
//!
//! 有两项曾经不摆——「加入合集…」与「加入子库…」——理由是**按下去没有下一步**：
//! 那两层弹层当时还不存在。票 `13`（收藏与合集）与票 `23`（从浏览屏加入子库）
//! 各自做出来之后，前提满足，两项补上了；`tests/menu.rs` 那条断言也从
...
pub enum Pressed {
    ...
    /// 收藏 / 取消收藏这一行。
    ToggleFavorite,
    /// **加入合集**：摊开「加入合集」那一层（票 `gui-looks-like-the-design/13` 做的那一层）。
    JoinCollection,
    /// **加入子库**：摊开「加入子库」那一层（票 `gui-looks-like-the-design/23` 做的那一层）。
    AddToSublibrary,
    /// 合并：勾中那一批，或者只有这一个。
    Merge,
    /// 刮削这一个作品底下那几个变体。
...
impl Facts {
    ...
    fn lines(&self) -> Vec<Line> {
        ...
        vec![
            ...
            Line::Rule,
            // **这两项照稿摆在这儿**（`ctxOpen` 那一行：`['coll',…],['sub',…]` 在分隔线之后、
            // 合并之前）。票 14 当初不摆它们，理由是「按下去没有下一步」——
            // 票 13 与票 23 各自把那一层做出来之后，那条理由不成立了（挂单 `Q1140` 收口）。
            item("加入合集…".to_owned(), "", Pressed::JoinCollection),
            item("加入子库…".to_owned(), "", Pressed::AddToSublibrary),
            item(
                if self.merging >= 2 {
