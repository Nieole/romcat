...
//! ## 为什么两态在这儿，而不在 [`App`] 里
//!
//! [`App`] 拿的是一份**已经开好的**现场，六屏全建立在「库一定在」这个前提上。让它自己
//! 兜住「还没有库」，就得把现场变成可空的，而那个可空会渗进六屏每一处判空——为的只是
...
enum Stage {
    ...
    /// **已开库**：现场开好了，主窗口在画。
    ///
    /// **装箱**：开场那一态几乎不占地方，主窗口那一态是整扇窗（六屏各自的缓存都在
...
impl Program {
    ...
    /// 而且进窗口之前还要把标题里那一段名字换掉（[`App::set_library_label`]）。
    ///
    ...
    pub fn ui(&mut self, ui: &mut egui::Ui) {
        ...
        match &mut self.stage {
            ...
            Stage::Opened(app) => {
                app.ui(ui);
                要回开场 = app.switching();
                换工作目录 = app.take_workspace_switch();
            }
        }
