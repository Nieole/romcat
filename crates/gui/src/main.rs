...
//! 直接开一份。**三样都不给就进开场**——列出这个工作目录里有哪些中立库，挑一份开进去
//! （ADR-0023：界面自足，从第一步起不必开终端）。**不擅自造一份合成的糊弄人**：
//! 假数据与真库在界面上长得一模一样，看见一屏假名字的第一反应会是「我的库怎么了」。
...
impl Args {
    ...
    /// 就进开场**（这条路从前是「都不给就悄悄造一份合成的」，再往后是「都不给就报错
    /// 退出」；合成数据与真库在界面上长得一模一样，于是不带参数打开看见的会是一屏假
    /// 名字，第一反应是「我的库怎么了」而不是「我打开的不是我的库」——**不报错的错比
    /// 报错的错难查得多**）。
    ///
