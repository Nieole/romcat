...
/// 字体的许可全文（SIL Open Font License 1.1）。
///
/// **跟着二进制走**。OFL 要求分发字体时随附许可，而这份字体是嵌进可执行文件的——许可
/// 若只躺在源码树里，被拷出去的那个可执行文件就不合规。界面的字体样张里看得到它。
pub const LICENSE: &str = include_str!("../assets/OFL.txt");
...
/// 会让本来对齐的哈希值与字节数错开。
///
/// 插在最前面不会挡住兜底：epaint 是**逐字符**沿回退链找字形的，子集里没有的字照样
/// 落到内置字体上。
