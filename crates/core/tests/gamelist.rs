...
fn 导出前就说得出这个格式的结构性损失_而且导入那一侧说的是同一句() {
    // ADR-0003 要的是「**导出前**就知道这个格式会丢掉什么」，而不是让用户导出一趟、
    // 回头发现两家开发商压成了一条、简介两头的空格没了，还以为是刮削出了问题。
    //
    // `分享包` 那一份 fixture 里**一样都没撞上**——八条声明照样得在两份报告里，
    // 因为它们说的是这个格式**结构上**做不到什么，不是这一趟丢了几条。
    let mut 现场 = 建现场();
    let path = 放一份分享包(&现场);
    let 导入报告 = 导入(&mut 现场, &path);
    let 导出报告 = 导出(&mut 现场);

    assert_eq!(
        导入报告.structural_losses,
        Gamelist.structural_losses(),
        "导入这一侧照搬适配器的声明"
    );
    assert_eq!(
    ...
    assert!(
        !导入报告.structural_losses.is_empty(),
        "空清单读作**还没查过**；量过了就不许再是空的"
    );

    for 报告 in [导入报告.render_text(), 导出报告.render_text()] {
        // 逐条挑一个别处不会出现的词——只钉其中两三条的话，把别的从清单里删掉
        // 这条测试照样绿，而它的名字说的是「这个格式的结构性损失」。
        assert!(报告.contains("U+3000"), "值两端的空白得点名：{报告}");
        assert!(
            报告.contains("desc") && 报告.contains("扩展键"),
            "不掐两端的那三样得说出口，省了就成了「简介的排版也会没」：{报告}"
        );
        assert!(
            报告.contains("分不开"),
            "两家与一家名字带逗号撞成一个：{报告}"
        ...
        assert!(
            报告.contains("unknown"),
            "字面上叫 `unknown` 的那一条：{报告}"
        );
        assert!(报告.contains("同一部作品"), "多文件条目摊开这件事：{报告}");
        assert!(
            报告.contains("启动命令"),
            "这个格式没有的那几个字段：{报告}"
        );
        assert!(报告.contains("合集段"), "合集段整段不见：{报告}");
        assert!(报告.contains("screenshot"), "认不得的资源槽：{报告}");
        assert!(报告.contains("第二个值"), "扩展键与未知元素：{报告}");
        // **绝不印一句「无结构性损失」的假保证**（空清单读作「还没查过」）。
        assert!(!报告.contains("无结构性损失"), "{报告}");
    }
}
