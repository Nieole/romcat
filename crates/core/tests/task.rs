...
#[test]
fn 识别按停之后在台上记成部分完成而且说得出下一趟接着算剩下的() {
    // **写过东西的活被叫停，得记成第三档**：识别起手就动过库（从头算的那一趟把上一轮的
    // 结论清干净，`Catalog::clear_identifications`），记成「已取消」的话，那句「可以当
    // 没跑过」是骗人的。
    //
    // **下一趟接着算剩下的**（票 `gui-answers-all-six/03`）。从前这一句如实说的是「识别
    // 没有断点，下一趟从头再算一遍」——那件事变了，这一句得跟着变：还说从头算的话，
    // 人会以为按停白按了，而那份结论其实留着、下一趟接着用。
    //
    // 把手**一开始就停着**：不赌「识别恰好还没跑完」，停下的那一刻钉在信号上。
    let 场 = 现场::摆好();
    let mut catalog = Catalog::open(&场.库文件).expect("能开中立库");
    let 弹 = 弹药::摆好();
    ...
    // 屏上那一句由 `Ending::render` 出，以收场那一档的词起头，不带 markdown 与 ADR 编号。
    let 屏上那一句 = ended.render();
    assert!(
        屏上那一句.starts_with("部分完成："),
        "收场那一档的词不对：{屏上那一句}",
    );
    assert!(
        !屏上那一句.contains("**") && !屏上那一句.contains("ADR-"),
        "屏上那一句带着 markdown 或 ADR 编号：{屏上那一句}",
    );
    let Ending::Halfway {
        product: interrupted,
        left_behind,
    ...
    assert!(
        left_behind.contains("下一趟接着算剩下的"),
        "说不出下一趟接着算：{left_behind}",
    );
    assert!(
        !left_behind.contains("从头再算"),
        "识别如今接着算，那一句还说从头再算：{left_behind}",
    );
