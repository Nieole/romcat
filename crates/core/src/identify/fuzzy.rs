...
/// 同一份数据源的**别名**那一路，在优先级表里另占一个名字。
///
/// **为什么要分成两个名字**：撞上一条条目之后，那条条目的中文名与它的**别名**都该进
/// 标题集合（用户搜哪个都该找得到），可它们在标题这一栏上的分量差得远——中文名是
/// 「这个文件的正题撞上的那个名字」，别名是「那条条目还叫什么」。优先级表是这个仓库
/// 里说「哪个源的哪个字段说了算」的地方（`scrape::priority`），把两路分开，它就能把
/// 别名排在**整条链的最后**：别名只进集合、只管搜得到。
///
/// **垫到最后这件事最终不是优先级表办到的**：那张表只在同一档之内分先后，而跨语言与
/// 类型的档位时它根本轮不到说话（`title::rank` 的层序）。真正拦住「别名当上显示标题」
/// 的是 `title::rank` 里按这个名字认出来的那一层——**集合里还有别的叫法时，它一律
/// 垫在最后**；一条别的叫法都没有时它照样当显示标题，那时有一个有出处的叫法总比一片
/// 空白强。
///
/// 合成一个名字的话，两者在库里是同一个源的两条值，排序上完全平手，最后是按字典序
/// 定的胜负——`合金彈頭7` 与 `合金弹头7` 谁当显示标题全看码位，那不是一条判据。
pub const ALIAS_SOURCE: &str = "中文离线源·别名";
