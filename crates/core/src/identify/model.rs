...
//!
//! ## 它的候选**照样算进结论的状态**，那句理由照留
//!
//! 状态在**全部候选都进来之后**才定：一次判断、一个候选集合
//! （[`State::of`](crate::catalog::identify::State::of)，票 `one-criterion-per-thing/06`）。
//! 一个只有这一层候选的变体落进**命中**——有候选、一条都没自动通过——与文件名那一层的
//! 候选同一个待遇（挂账 D127）。从前它照旧记成「未命中」或「无判据」，库里于是写着
//! 「它一条候选都没有」，而它说了，只是说的人是模型。
//!
//! 两件事因此要另外守住：
//!
//! 1. **「无判据」那一列的理由是真事实**（`容器穿不透`、
//!    `zst 穿不透，而这一趟没为它付全量解压的代价`、`元数据读不到`）。真机上残渣里
//!    有 3,237 个是这一档。理由在这一层之前就挑好了，这一层的候选与它**并存**，
//!    一个字不盖掉。
//! 2. **命中率不许替它邀功**：报告里「只靠名字」那一列把这一层也算进去
//!    （`identify::report` 的 `BLIND_SOURCES`），「不算那几层的命中率」照旧回答
//!    「这个库被**认出来**多少」。
//!
//! ⚠️ 这一改让**历史库的状态计数跳一次**（只有模型候选的变体从未命中、无判据挪进命中），
//! 重跑一趟识别就对上。结构版本不升：表与列一个没改，改的是这一列怎么填。
//!
