...
pub struct PlatformRow {
    ...
    pub cjk_named: u64,
    /// 命中里**候选全出自只靠名字那几层**的变体数：文件名那一层（票 11）与模型推断那一层
    /// （票 12；它的候选从票 `one-criterion-per-thing/06` 起也算进状态）。
    ///
    /// 它单列一列，因为它改变「命中」这两个字的含义：那几层的候选一条都不自动通过，
    /// 它们只是**给了一条带依据的候选让人去裁**。把它混进命中率里，读者就分不出
    /// 「认出来了」与「有人按名字猜了一下」——而那正是这份报告一直在防的事
    /// （跳过与无判据不混进未命中，是同一条道理）。
    pub name_only: u64,
    /// 上面那些里，**本来是无判据**的有几个（一份可以撞的内容都没有）。
    ///
    /// 「不算只靠名字那几层的命中率」要靠它把分母也还原回去——无判据本来就不在命中率的
    /// 分母里，只减分子不减分母，算出来的数会比那几层跑之前还低。
...
///
/// 它们共用「只靠名字」那一列，因为它们是同一件事：都只看名字（模型推断那一层另外看
/// 前面几层已经读出来的头部字段与同目录的名字，自己一个字节都不读）、都永不自动通过。
/// 少列一个，那一列就会让「命中」两个字被只看名字的层悄悄撑起来——而这一列存在的
/// 全部理由正是防这件事（同「跳过与无判据不混进未命中」）。模型推断那一层从票
/// `one-criterion-per-thing/06` 起候选也算进状态，于是它也得在这儿。
/// 源名写死在这儿而不是拼出来，是因为 SQL 要的是一个 `&'static str`；
/// **有一条测试钉着它与那几个常量一致**，改了名字而忘了改这里，测试当场红。
const BLIND_SOURCES: &str = ",中文离线源,Switch 文件名,模型推断,";
...
impl IdentifyReport {
    ...
    pub fn render_text(&self) -> String {
        ...
        if self.total.name_only > 0 {
            let _ = writeln!(
                out,
                "                其中 {} 个**候选全出自只靠名字的那几层**（文件名、模型推断——\
                 一条都不自动通过，全部等裁决；其中 {} 个本来是无判据的）——不算那几层，\
                 撞了 DAT 的里面是 {:.1}%",
