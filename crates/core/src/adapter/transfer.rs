...
pub enum ExportError {
    ...
    #[error("导出已取消：还没开始往盘上写，一份元数据文件都没动。")]
    Halted(#[from] Halted),
}
...
///
/// [`Catalog::mark_exported`] 在这一趟的**末尾**、跳过了 `dry_run`、被按停与
/// **撞上外面有人动过、有一份没写**的那三种之后才落。库屏工序段上导出那一行说的
/// 正是它——那一支的「还差多少」算不出来（`stage::Stage::Export` 写着为什么与实测
/// 代价），退回显示上次跑的时刻。只排了计划、或者只写了一半就说「上次跑是刚刚」，
/// 那一行就在骗人（规格第 31 条「至少不骗我」）。
///
/// # Errors
...
pub fn export_task(
    ...
    } else if 留下了东西 && report.conflicts.is_empty() {
        // **记下这一趟导出的时刻**：库屏工序段上导出那一行说的正是它。
        //
        // **判据是「该写的每一份都真写到了盘上」**，这几种因此都不打戳：只排计划那一趟
        // （`dry_run`，一个字节都没写）、被按停那一趟（上面那一支），以及**撞上外面有人
        // 动过、有一份没写**的那一趟——哪怕别的几份写成了。最后这一种最容易漏：它走的是
        // 正常出口、报告也没有 `interrupted`，可人要的那一份还躺在盘上等他去看；
