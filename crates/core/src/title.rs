...
fn classify(
    ...
    if source == crate::identify::fuzzy::ALIAS_SOURCE {
        return Classified {
            ...
            // 什么**——没有任何东西说过这个文件叫它。
            //
            // 这一档同时是「别名不当显示标题」唯一真管用的那道闸。[`rank`] 的排序键里
            // **置信度排在优先级表名次前面**，所以光把它在 `priorities.toml` 里垫到最后
            // 是拦不住的：语言与类型一样时，中置信压过一切低置信的叫法，包括这个变体
            // 自己的文件名。降到低置信之后它与文件名同档，再由那张表分先后——
            // 而那张表里 `文件名` 排在 `中文离线源·别名` 前面。
