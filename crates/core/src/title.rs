...
/// 不在这张表里排——它整路垫在表外，理由见下面第 2 层。
///
/// ## 同一部作品有好几个叫法时挑哪一个
///
/// **确定的选定规则**，七层（`rank`），缺一不可。这一段是那条规则的**唯一出处**——
/// 报告与文档都从这里抄，抄岔了两处就会各说一套：
///
/// 1. **人工裁决**——人改过的东西不许被任何数据源覆盖；
/// 2. **是不是「那条条目还叫什么」**——中文离线源那一次撞上的条目的别的叫法，进集合
///    只为**搜得到**，一律垫在最后。**这一层排在档位前面**，因为它要拦的是**跨档**
///    的那一种：`name_cn` 是一串拉丁字时正名落到英文那几档，而它的中文别名落进 3 档，
///    档位一分先后，后面的置信度就再没机会说话了；
/// 3. **语言与类型的档位**——就是上面那张表；
/// 4. **置信度**——官中那一条压过没人背书的文件名；
/// 5. **优先级表里的源名次**（`标题` 那一栏）——已经有一份配置好的表，不另发明一套。
///    **它排在数量之前**：`priorities.toml` 说 No-Intro 的条目名最整齐，那就不该被
///    「同一个 TOSEC 写法在盘上重复了三十遍」压过去；
/// 6. **有几个变体这么叫**，多的优先——同一档、同一个源之内，几十个文件都这么写的
///    那个比孤零零一个的可信；
/// 7. **字典序**——前六层平手时定死顺序，同一份库跑两次挑出来的必须是同一条。
#[must_use]
...
/// （裁决优先 → 「那条条目还叫什么」垫底 → 语言与类型档位 → 置信度 → 源名次 →
/// 有几个变体这么叫 → 字典序）。
///
/// 集合里一条中文叫法都没有时是 `None`。
///
/// ## 它答的是「中文那一档的第一名」，**不是**「显示标题」
///
/// 两者在两种情形下会分开，摆给人看的时候要说清是哪一种，别写成「前端里会看到这个」：
///
/// 1. **人裁过一个非中文的叫法**——裁决压过一切（`rank` 的第 1 层），显示标题是
///    人挑的那个；
/// 2. **中文那一档里只剩「那条条目还叫什么」**（[`fuzzy::ALIAS_SOURCE`](crate::identify::fuzzy::ALIAS_SOURCE)）——
///    它整路垫在档位之前（第 2 层），所以显示标题会是这个文件**自己撞上的**那个名字，
///    哪怕那是一串拉丁字。西方作品那一类常常正好如此（`name_cn = "Minecraft"` 而
///    别名里躺着 `我的世界`）。
///
/// 这两种情形下这个函数照旧交出那条中文叫法——**它确实是这个作品的一个中文叫法**，
/// 界面要说得出「有」还是「没有」，用户也要搜得到。
#[must_use]
...
fn rank(entry: &TitleRow, priorities: &Priorities) -> Rank {
    ...
    (
        ...
        // **「那条条目还叫什么」垫在语言那一档之前**（[`fuzzy::ALIAS_SOURCE`]）。
        //
        // 这一层非在 `bucket` 前面不可，因为它要拦的正是**跨档**的那一种：中文数据源
        // 对西方作品常有 `name_cn` 就是一串拉丁字的条目（`name_cn = "Minecraft"`），
        // 那时这个文件**自己撞上的**那个名字按字形判成英文、落到 5 桶开外，而同一条
        // 条目的中文别名（`我的世界`）落进 3 桶——桶排在置信度前面，于是那条**低置信**
        // 的别名压过了中置信的正名，还压过 4 桶的汉化名。摆在 `bucket` 后面这一层就
        // 只在同档之内说话，那正是它拦不住的那一种。
        //
        ...
        //
        // 垫到最后不是扔掉：这一路照旧入集合（**搜得到**），而且集合里一条别的叫法都
        // 没有时它照样当显示标题——那时有一个有出处的叫法总比一片空白强。
        u8::from(entry.source == crate::identify::fuzzy::ALIAS_SOURCE),
        bucket,
...
/// [`rank`] 的排序键。七层，含义见 [`choose`]。
///
/// 起个名字而不是把七元组摊在签名里：`choose` 有两处按它排序（挑显示标题、挑排序标题
/// 用的拉丁标题），两处的类型必须一模一样。
...
fn classify(
    ...
    if source == crate::identify::fuzzy::SOURCE {
        return Classified {
            ...
            //
            // **中置信不等于不用看**：没人裁过，它照样进标题这一侧的待确认队列
            // （`title::report`，挂账 `D128`）——进不进队列不看这一档。
    ...
    // **同一次撞上的那条条目还叫什么**（票 01）。它进标题集合是为了**搜得到**，
    // 不是为了当显示标题。
    if source == crate::identify::fuzzy::ALIAS_SOURCE {
        return Classified {
            kind: TitleKind::Alias,
            // **低置信，比上面那一条低一档**，而这一档差是这条值真正该有的分量：
            // 上面那一条是**这个文件的正题撞上的那个名字**，这一条只是那条条目**还叫
            // 什么**——没有任何东西说过这个文件叫它。
            //
            // 这一档管的只是**同一档之内**的先后。[`rank`] 的排序键里**置信度排在优先级
            // 表名次前面**，所以光把它在 `priorities.toml` 里垫到最后是拦不住的：语言与
            // 类型一样时，中置信压过一切低置信的叫法，包括这个变体自己的文件名。降到低
            // 置信之后它与文件名同档，再由那张表分先后——而那张表里 `文件名` 排在
            // `中文离线源·别名` 前面。
            //
            // **「别名不当显示标题」这件事不靠这一档。** 置信度与那张表都排在语言与类型
            // 的档位后面，跨档时根本轮不到它们说话（`name_cn` 是一串拉丁字时正名落到
            // 英文那几档，中文别名落进第 3 档，别名就赢了）。真管用的那道闸在 [`rank`]
            // 的**第 2 层**，它按来源认出这一路、整路垫在档位之前。
            confidence: Confidence::Low,
            seam: None,
            evidence: "这是同一次撞上的那条条目的另一个叫法——\
                       进标题集合是为了搜得到，没有任何东西说过这个文件叫它"
                .to_string(),
        };
    }
    ...
    Classified {
        ...
        // 合集包、精简版、带广告后缀的文件名全落在这一档。它进待确认队列**不是因为这一档**，
        // 是因为没人裁过（`title::report`，挂账 `D128`）。
        confidence: Confidence::Low,
        seam: None,
...
mod tests {
    ...
    #[test]
    fn 中文名其实是拉丁串时低置信别名不当显示标题() {
        // 中文数据源对西方作品常有 `name_cn` 就是一串拉丁字的条目。那时这个文件
        // **自己撞上的**那个名字按字形判成英文、落到第 5 档开外，而同一条条目的中文
        // 别名落进第 3 档——档位排在置信度前面，于是一条**低置信**的别名压过了中置信
        // 的正名。这一条钉的就是「那不该发生」。
        let priorities = Priorities::builtin();
        let mut 正名 = 叫法(
            "Minecraft",
        ...
        正名.confidence = Confidence::Medium;
        let 那条条目还叫什么 = 叫法(
            "我的世界",
            Language::Chinese,
            TitleKind::Alias,
            crate::identify::fuzzy::ALIAS_SOURCE,
        );
        let 文件名 = 叫法(
            "Minecraft v1.16",
            Language::English,
        ...
        let set = 集合(vec![那条条目还叫什么.clone(), 文件名.clone(), 正名.clone()]);
        let chosen = choose(&set, &priorities);
        assert_eq!(
            chosen.display, "Minecraft",
            "显示标题该是这个文件自己撞上的那个名字，不是那条条目的另一个写法"
        );
        // **别名照旧在集合里**——`choose` 一条都没扔，用户搜「我的世界」还搜得到。
        assert_eq!(set.entries.len(), 3);
        assert_eq!(chosen.chinese_names, 1, "那个中文叫法还在集合里数着");

        // 这个变体自己的文件名也压得过它：两条都是低置信的别名，一条是盘上这个文件
        // 真叫的名字，另一条只是那条条目还叫什么——**跨语言也一样**，从前正是这一步
        // 跨不过去（文件名是英文、落第 7 档，别名是中文、落第 3 档）。
        let 只有两条 = 集合(vec![那条条目还叫什么.clone(), 文件名]);
        assert_eq!(choose(&只有两条, &priorities).display, "Minecraft v1.16");

        // **一条别的叫法都没有时它照样当显示标题**：那时有一个有出处的叫法总比一片
        // 空白强（`priorities.toml` 标题那一节写着同一句）。
        let 只剩它 = 集合(vec![那条条目还叫什么.clone()]);
        assert_eq!(choose(&只剩它, &priorities).display, "我的世界");

        // 它垫得再靠后也压不过**裁决**——第 2 层在第 1 层之下。
        let mut 人说的 = 叫法(
            "我的世界",
            Language::Chinese,
            TitleKind::Translated,
        ...
        let 有裁决 = 集合(vec![那条条目还叫什么.clone(), 正名.clone(), 人说的]);
        let 裁过的 = choose(&有裁决, &priorities);
        assert_eq!(裁过的.display, "我的世界");
        assert_eq!(裁过的.kind, Some(TitleKind::Translated));

        // **「中文优先」这条回退链这一票一个字没改。** 同一部作品真有汉化版时，
        // 中文的汉化名照旧压过拉丁的正名（第 3 层的档位，4 档 < 7 档）——变的只是
        // 那条别名不再压在汉化名头上（`ADR-0012` 那条「别拿汉化名当标题」管的是
        // **中文那一档之内**的先后，不是把中文整档让给英文）。
        let mut 汉化 = 叫法(
            "我的世界 某某汉化组版",
            Language::Chinese,
            TitleKind::FanName,
