...
/// 「前几批可一次处理多少」按**前几批**算：库屏工序段裁决那一行底下那句（设计稿「前 5 批可一次处理 12,223 个」）与待确认队列屏
/// 头一格「前 5 批可直接批量处理」说的是同一个数。前几批**只数能整批通过的**（[`batch::Coverage`]）。
...
///
/// 下一趟识别会照沉淀库把中立库这一半重放一遍，结果与这里写下的一模一样
/// （[`Projector`] 两处共用）——结论、候选与发行版整批清掉重建，作品那一行按名字复用
/// （[`Catalog::clear_identifications`](crate::catalog::Catalog::clear_identifications)）。
///
/// ## 落下的同时记成一**批**
///
