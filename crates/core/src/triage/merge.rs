...
//! `triage::resolve` 时造的是「只有作品名与变体自己的平台」那一份事实——
...
/// **这个变体本来就挂在保留的那个作品名下**——它不成其为「归入」，[`plan`] 把它略过。
///
/// 这一句**只许有一处**（ADR-0005 再修订那一节）：屏上第二步那一列常驻写着它，
/// 那几行的勾选框画成灰的、停上去说的也是它。各写一份的话，屏上那句与真正挡住的那一处
/// 迟早分叉——而分叉的样子是「屏上说它自带，实际却被当成一条要落的裁决」。
pub const ALREADY_HELD: &str = "保留作品自带";
...
pub fn plan(
    ...
    for key in moving {
        ...
        match row.work_id.and_then(|id| works.get(&id)) {
            // **已经在它名下**（[`ALREADY_HELD`]）——它不成其为「归入」，再裁一遍只是多一条
            // 一模一样的裁决。略过而不是拒绝：人要的结果**本来就成立**；真要是略到一个不剩，
            // 底下那句 [`MergeError::Nothing`] 会连原因一起说出来。
