...
//! 再加一道防炸的闸：同一串字（或同一条条目）底下挤着 [`CROWDED`] 个以上作品时整堆不提。
//! 这三句与词表**疑似同一作品**那一条逐字同一句。
//!
...
//! 再逐对问年份——与 `Catalog::facets` 同量级，真库上没量过。**算一次、攥在手里**：
//! 库体检那一趟在后台那条线程上算（`health::check_run` 走 [`survey_apart`]），
//! 浏览屏在开窗与库变过之后各算一次（挂单 `Q1122`：它眼下还在界面那条线程上，
//! 该与 `Q1011` 一趟挪上任务台）。
