...
//! ## 锚点为什么是字符串而不是行号
//!
//! **识别与刮削是两个可分别重跑的阶段。** 重跑识别会把它自己造的**发行版**整批删掉再造
//! 一遍（[`Catalog::clear_identifications`](super::Catalog::clear_identifications)），
//! 新造出来的行拿的是新的 `INTEGER PRIMARY KEY`；**作品**那张表票 parking-3/10 起改成
//! 按名字复用，行号跨重跑不换了，但一部作品底下最后一个变体没了、或者这一趟一条判据都
//! 没落在它身上时，那一行照样收得掉、下次回来拿新号
//! （[`Catalog::drop_unheld_identified_works`](super::Catalog::drop_unheld_identified_works)）。
//! 刮削结论若挂在行号上，那几种情形下就全成了孤儿——在线档那边等于把配额烧掉重来。
//!
//! 所以锚点是**自然键**：作品锚点是**作品名**（那正是识别给作品去重、也正是复用现成那一行
//! 用的键），变体锚点是**变体的键**（相对主库根的路径，ADR-0020）。两者都不随识别重跑而变。
