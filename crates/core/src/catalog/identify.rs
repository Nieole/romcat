...
/// 而实情是工具**一个字都没说**。
///
/// 收在核心库里而不是各屏各写一份：六屏都要标这四档，含义漂开一点点，用户就再也
...
///
/// 六屏里真会标置信度的是**浏览**与**待确认**两屏：库屏摆的是根与数据源、子库屏摆的是
...
pub enum Tier {
    ...
    ///
    /// **两个词在六屏上分开印**（票 `gui-redesign/17`）：待确认屏的屏头把两个数并排说，
...
/// 撞在一起。现在那一档叫「**没有候选**」，六屏同用一个词（`Tier::label`），
...
impl Catalog {
    ...
    ///
    /// 识别判过的，就是它**判定的那个**（`identification.platform`）：人在**平台纠正**上
    /// 按组定过的听人的，没定过的看卡带内部头，一个都读不出才是目录声明的——那条回退链
    /// 只在一处（`identify::platform_of`，ADR-0024）。**识别没判过的**——还没识别，或者是加
    /// 那一列之前识别的——手上只有目录声明的那个。
    ///
    /// **这一列是「按哪个平台算」的结论，不是「谁说的」**：三样来源分得出来，各有各的出处
    /// ——人定的那一条在**沉淀库**的 `platform_correction` 表里，内容说的在 `content_cart`
    /// 上，目录说的原样在 [`VariantRow::platform`](super::VariantRow::platform) 上。
    /// 物化一份结论是 ADR-0024 推论 3 允许的，被禁的是第二次**判断**。
    ///
