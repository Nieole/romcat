...
//! - **年份与评分是「一列文字读成数」**，而「什么算一个年份」只有一份定义，写在
//!   [`parse_year`](crate::sublibrary::rule::parse_year) 里；这边那句
//!   [`YEAR_VALUE`] 是它的 SQL 译文，读不成年份的值两侧一致地当成「这一维上没有值」。
//!
...
/// 把年份那一列文字读成数。**判据不在这儿定，在
/// [`parse_year`](crate::sublibrary::rule::parse_year) 那儿**——这一句只是把它翻成 SQL。
///
/// 那边写的是「前后空格去掉之后恰好四位 ASCII 数字」，这边就是一句
/// `GLOB '[0-9][0-9][0-9][0-9]'`（`GLOB` 整串匹配、大小写不折，四个字符一个不多一个不少），
/// 加上 SQLite 的 `trim()`——它**只去空格**，所以那边配的是 `trim_matches(' ')` 而不是
/// `str::trim`。
///
/// 从前这里是「`CAST` 往返回得到原文就算数」，而内存那一侧是 `parse::<i32>()`。
/// 两句话看着都像「读得成整数」，实际放行的不是同一批：`01996` 与 `+1996` 往返回不到原文，
/// `parse` 却收得下。于是「年份=1996」屏上筛出两个、按下同步搬走四个——正是模块文档
/// 开头那条唯一的硬约束。判据只留一份之后，这类分家没有地方再发生。
const YEAR_VALUE: &str = "CASE WHEN trim(sv.value) GLOB '[0-9][0-9][0-9][0-9]'
