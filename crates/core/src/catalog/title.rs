...
//! 重跑识别会把它自己造的发行版整批删掉再造一遍，新行拿的是新的
//! `INTEGER PRIMARY KEY`；作品那张表票 parking-3/10 起按名字复用，行号跨重跑不换，
//! 但那一行收得掉的那几种情形照旧在（底下最后一个变体没了、这一趟一条判据都没落在
//! 它身上）。标题集合若挂在行号上，那时就成了孤儿，而外键是开着的——那还不是
//! 「留下孤儿」，是删那一行时当场报错。
//!
...
CREATE TABLE IF NOT EXISTS title(
    -- 锚点是**作品名**而不是行号：作品那一行收得掉、收掉再回来就是新号
    -- （同 scrape_value，见本模块开头）。
    work        TEXT    NOT NULL,
...
impl Catalog {
    ...
    pub fn titles_folded_at(&self) -> Result<Option<i64>, CatalogError> {
        Ok(self
            .meta_get(MetaKey::TitlesFoldedAt)?
            .and_then(|value| value.trim().parse::<i64>().ok()))
    }

    /// 记下**这一趟重折**的时刻。
    ///
    /// [`crate::title::run_task`] 走完一整趟才调它，于是命令行 `romcat titles` 与界面上
    /// 工序段那一行记下的是同一件事——**有人真跑了这道工序**，不是「手上这份集合有多新」。
    /// 一条否定裁决就地重折（`scrape::zh::judge` → [`crate::title::refold`]）**不打这个戳**：
    /// 人没点过那颗按钮，屏上就不该说「上次跑是刚刚」（挂单 `Q428`、`Q440`）。
    ///
    /// # Errors
    /// 写库失败时返回错误。
