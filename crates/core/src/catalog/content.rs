...
--
-- **`name` 上没有 `UNIQUE`，那是有意的**（票 parking-3/10、挂账 D162）：作品是**游戏
-- 概念**，平台在发行版那一层，所以同名异作是真实存在的（1988 与 2004 两部 Ninja
-- Gaiden）。裸约束会把它们强行并成一部，连日后人工把其中一部拆出来的路都堵死。
-- 「同一部作品只该有一行」这件事**由复用那条路保证**（`identify::Projector::work` 按
-- 名字找现成的那一行），不由约束保证。
...
--
-- 加索引不改任何一行存的东西，所以**不动 `SCHEMA_VERSION`**——`CREATE INDEX IF NOT
-- EXISTS` 每次开库都跑一遍（`Catalog::open` 里那几句 `batch`），老库照样补得上，
-- 与 `identify::add_columns` 同一条路。
CREATE INDEX IF NOT EXISTS work_name ON work(name);
...
impl Catalog {
    ...
    /// （`drop_variant_orphans` 上写着判据与
    /// 那份清单为什么是这几张表）。
    ///
    /// `scan` 是这次成型对着的遍历代号，记进 `meta`，报告据此说得出「成型是不是比库旧」。
    /// **它是全库最新那一个代号，不是「这一趟扫的那个根」的**：成型是整份库上的一遍纯
    /// 计算（ADR-0022），输入里没有「根」这个概念，而读它的报告比的也是
    /// [`Catalog::last_traversal`]。按根续扫时那两个数会分家——断点里的代号早被中间
    /// 扫过的别的根推走了——**分家的时候记的是全局那一个**，由调用方取
    /// （`scan::scan` 与 `romcat shape` 两处都走 `last_traversal`）。
    ///
    /// **这里不替调用方取 `max(旧值, scan)` 兜底。** 那是在最里面一层把一个传错的代号
    /// 盖住，而「这遍成型对着的库有多新」是调用方才知道的事；盖住之后，真要往回记
    /// 一个更旧的代号（库被换过、结构版本回退）就再也记不进去了。
    ///
    /// # Errors
    /// 写库失败时返回错误。
    ...
    /// 记一个**作品**，返回它的 id。**这是新建那一支**——先问过
    /// [`Catalog::work_named`](super::Catalog::work_named) 没有现成的行才走到这儿
    /// （`identify::Projector::work`），否则同一部作品会攒出两行。
    ///
    /// `origin` 说这一行是谁造的。识别重跑时只收自己造的、如今没人再指着的那些，见
    ...
    ///
    /// **只往「裁决」这一档写。** 唯一的调用方（`identify::Projector::work`）在识别
    /// 认领到一行 `origin = 裁决` 时一个字都不动它：那一档说的是「沉淀库那边有一条钉着
    /// 它」，而反着降会在识别与裁决两次认领之间被中断时把它永久写坏（那一头的文档
    /// 「来路只升不降」那一段说的就是这件事）。
    ///
    /// # Errors
    /// 写库失败时返回错误。
    ...
    /// 全部**作品**：id → 名字。
    ///
    /// 刮削拿它做**锚点**：作品锚点是**作品名**而不是行号。行号票 parking-3/10 起跨
    /// 识别重跑不换了，但一行收得掉的那几种情形照旧在，所以锚点照旧是名字
    /// （`catalog::scrape` 的模块文档说的就是这件事）。
    ///
    /// # Errors
    /// 读库失败时返回错误。
