...
-- 会变成一条能撞上《任天堂》的「叫法」。
--
-- `ord` 是数据源里的**原次序**，不是排序用的装饰。⚠️ **导出那条链上仍旧没有人读它**：
-- 开发商、发行商与类型现在按集合导出（`converge::build_game` 的 `pick_all`），
-- 「换了一家公司」那个错已经没了——`|开发= 科乐美、KCE东京` 两家都写得出去；
-- 但**先后仍是码位序**（`Catalog::for_each_scraped_value` 的排序键），因为
-- `scrape_value` 上没有对应的一列。这一格留着不是白留：要把原次序一路带到前端，
-- 就得让它跨过中立库那一层。记在挂单 Q27。
...
mod tests {
    ...
    #[test]
    fn 中文离线源的连接设了等锁的余量() {
        let dir = crate::testing::temp_dir("zh-store-timeout");
        let store = Store::open(&dir.path().join("zh.sqlite3")).expect("开得起来");
        let 余量: i64 = store
            .conn
    ...
    #[test]
    fn 写锁占着时中文离线源等得到而不是当场报忙() {
        let dir = crate::testing::temp_dir("zh-store-busy");
        let path = dir.path().join("zh.sqlite3");
