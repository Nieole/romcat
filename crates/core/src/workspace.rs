...
impl<'a> Slug<'a> {
    ...
    /// 中立库；而这里一处改完，连**沉淀库**里那些**路径锚**记的**主库标识**也跟着一起对上了
...
/// 同上，只是主库那一半**已经折成了主库标识**（[`Slug::text`] 的产物）。
///
/// 开完一份现场之后，手上剩的就是那串主库标识而不是 [`Slug`]
/// （`site::Site::library_identity`）。再包一次 [`Slug::Named`] 会哈希两遍、折出第二个文件名，
