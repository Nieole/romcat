...
//!
//! [`StructuralLoss`] 补的正是这一半：每个适配器把自己格式的**结构性损失**逐条
//! 声明出来，[`Adapter::structural_losses`] 答。ADR-0003 要的「导出前就知道这个
...
/// **结构性损失**里的一条：把一个值交给这个格式存一趟，它**自己**会把值改成什么样。
///
...
pub trait Adapter {
    ...
    /// 这个格式的**结构性损失**——[`Capability`] 那四个词说不出的那一半。
    ///
