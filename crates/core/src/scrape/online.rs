...
//!
//! 离线源补不上的只剩**封面**——本地数据源里根本没有图片（ADR-0007）。简介、类型、
//! 开发商与发行商票 02 至票 04 之后离线档自己都有了；在线档如今就是为图存在的。
//!
//! ## 这一档对这个库有结构性风险
//!
...
/// **ScreenScraper 源**：按哈希查，补**封面**——离线档补不上的只剩它。
///
/// 它照旧一并交出简介、类型与开发商（一次请求本来就带回来了，多一栏不多发一个请求），
/// 只是那几栏离线档自己也有了，在优先级表上各按各的名次排。
///
