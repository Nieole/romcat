...
//! ## 一条规则是一棵**组**
//!
//! 每个组挑一种**组合方式**：`全部满足` / `任一满足` / `都不满足`；组里摆的既可以是
//! **子句**，也可以是另一个组。于是它嵌得下去：
//!
//! ```text
...
//!
//! - **组合方式两侧要有空白**（`且` `或` 都一样），于是值里出现这个字也不会被当成分隔符
...
impl Dimension {
    ...
    pub fn hint(self) -> &'static str {
        match self {
            ...
            Self::Rating => "评分，写 0–1 的小数或 `80%`——眼下没有源，筛不出东西",
    ...
    /// 这一维**眼下没有源**——写得出规则，可中立库里还没有哪个源在写它，所以筛不出东西。
    ///
    /// **只有这一处答这件事**（ADR-0024）：`评分` 没有源这句话本来散在三处（本枚举的
    /// 文档、`Dimension::hint` 那半句、以及选择集报告点名那一档），D68 真落地那天要改三处，
    /// 漏一处就是屏上还在说「筛不出东西」而它其实筛得出来了。
    ///
    /// 与[选择集报告](super::Selected::thin_dimensions)那一档**问的不是同一件事**：
    /// 那一个是**求完值**答「这一维在这份库里一条数据都没有」（可能只是这份库还没刮到），
    /// 这一个是**整个本仓还没有这个源**，与手上是哪份库无关。
    #[must_use]
...
impl Op {
    ...
    /// 九个运算符的固定次序，界面照它摆。
    ///
    /// **哪个符号配得上哪个维度不由这个次序说了算**，由 `fits` 说了算：文字维度
    /// 正好是前五个，数值维度是除掉那三个字串符号（`~` `^` `$`）之外的六个。
...
pub enum RuleError {
    ...
    /// 值里有会被读成分隔符的组合方式：前面是空白，后面是空白或者正好到值的末尾。
    #[error(
        "{dimension} 的值「{value}」里的 `{joiner}` 会被读成组合方式——它前面是空白，\
...
impl Rule {
    ...
    ///
    /// 人没起过名字时屏上就用这个（起过的那一档见 `StoredRule::shown_name`，
    /// 票 `gui-looks-like-the-design/23` 给中立库补了 `name` 那一列，挂单 `Q811` 收口）——
