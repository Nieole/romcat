...
//! 重跑识别照沉淀库重放一遍，结果一模一样（发行版整批重建，作品按名字复用现成的那一行
//! ——见 [`Catalog::clear_identifications`](crate::catalog::Catalog::clear_identifications)）。
//!
//! ## 这份库自己怎么迁移
//!
...
//!   哪条旧键属于哪个根，这份库里没有任何判据说得出来（它只记着**主库标识**，
//!   记不着那时候只有一个根）。
...
impl Index {
    ...
    /// 把一份沉淀库整个读进来。`library` 是这一趟对着的**主库标识**，**只有它的路径锚算数**。
    ///
    /// # Errors
    /// 读库失败时返回错误。
...
impl MatchIndex {
    ...
    /// 把一份沉淀库里的匹配裁决整个读进来。
    ///
    /// `library` 是这一趟对着的**主库标识**，**只有它的路径锚算数**（同 [`Index::load`]）。
    ///
    /// # Errors
    /// 读库失败时返回错误。
    pub fn load(store: &Store, library: &str) -> Result<Self, VerdictError> {
...
mod tests {
    ...
    fn 路径锚的成员关系只在自己那份主库里算数() {
        ...
        // 说的就不是同一个东西了。快照按主库标识筛，投影才不会把甲库的收藏落到乙库上。
        let mut store = Store::in_memory().expect("开得出来");
